function [hshaft, hhead, hcap_shaft, hcap_head] = arrow3_hsvcolor_batch_uniform(X, Y, Z, U, V, W, varargin)
% ARROW3_HSVCOLOR_BATCH_UNIFORM
% 批量绘制三维箭头（圆柱+圆锥+端盖），统一尺度，方向相关着色。
% 新增参数：'Scale'  —— 对形状做等比例整体缩放（总长、半径、端盖间隙等全部按比例缩放）。
%
% 颜色规则：+z 白，-z 黑，水平用 HSV 的 Hue；以 w=sin(theta) 混合灰度与纯色。
%
% 用法：
%   [hS, hH, hCapS, hCapH] = arrow3_hsvcolor_batch_uniform(X,Y,Z,U,V,W, ...
%       'Length', Lbase, 'Scale', 1.0, 'Radius', [], 'HeadFrac', 0.25, ...
%       'HeadRadiusFactor', 2.2, 'Facets', 24, 'EdgeColor', 'none', ...
%       'CapEpsilon', 1e-6);
%
% 关键参数：
%   'Scale'            : 等比例整体缩放（默认 1.0）。会同时缩放 Length、Radius、端盖间隙等。
%   'HeadFrac'         : 圆锥长度占比（0~1），大→锥长、柱短（默认 0.25）
%   'HeadRadiusFactor' : 圆锥底半径/圆柱半径 的比例（默认 2.2，越大越粗）
%
% 其他参数：
%   'Length'   : 基准总长度（base length）；若未给出，取非零向量模的中位数
%   'Radius'   : 基准圆柱半径；若未给出，默认 0.03 * Length
%   'Facets'   : 圆周分段（默认 24）
%   'EdgeColor': 边线颜色（默认 'none'）
%   'CapEpsilon': 端盖共面闪烁避免的微小偏移（相对“有效总长”的比例，默认 1e-6）
%
% 返回：
%   hshaft    : 圆柱侧壁 patch
%   hhead     : 圆锥侧壁 patch
...
for t = 1:Nv
    ...
    C_shaft     (ofsFc + (1:nFc), :) = repmat(col, nFc, 1);
    ...
    C_head     (ofsFh + (1:nFh), :) = repmat(col, nFh, 1);
    ...
    C_cap_shaft     (ofsFcs + (1:nFd), :) = repmat(col, nFd, 1);
    ...
    C_cap_head     (ofsFch + (1:nFd), :) = repmat(col, nFd, 1);
...
hshaft = patch('Faces',F_all_shaft,'Vertices',V_all_shaft, ...
    'FaceVertexCData',C_shaft,'FaceColor','flat','EdgeColor',edgeCol, ...
    'SpecularStrength',0.3);

hhead  = patch('Faces',F_all_head ,'Vertices',V_all_head , ...
    'FaceVertexCData',C_head ,'FaceColor','flat','EdgeColor',edgeCol, ...
    'SpecularStrength',0.35);

hcap_shaft = patch('Faces',F_all_cap_shaft,'Vertices',V_all_cap_shaft, ...
    'FaceVertexCData',C_cap_shaft,'FaceColor','flat','EdgeColor',edgeCol, ...
    'SpecularStrength',0.3);

hcap_head  = patch('Faces',F_all_cap_head,'Vertices',V_all_cap_head, ...
    'FaceVertexCData',C_cap_head,'FaceColor','flat','EdgeColor',edgeCol, ...
    'SpecularStrength',0.35);
...
for k=1:numel(args)
    a = args{k};
    if isscalar(a), args{k} = repmat(a, sz);
    else, assert(isequal(size(a), sz), '所有输入必须同尺寸或标量。');
    end
end
...
function rgb = dir2rgb(d)
dz = d(3);
theta = acos(max(-1,min(1,dz)));
phi   = atan2(d(2), d(1)); phi = mod(phi, 2*pi);
w = sin(theta);
g = (cos(theta)+1)/2;
h = phi/(2*pi);
pure = hsv2rgb([h, 1, 1]);
rgb = (1-w)*[g g g] + w*pure;
rgb = max(0,min(1,rgb));
end
